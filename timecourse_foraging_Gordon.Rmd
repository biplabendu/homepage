...
# Classification of observation days as wet/average/dry days
clim.group.ids <- c("wet-day","average-day","dry-day")
```
...
# src_dbi(climate.db)
clim.dat.all <- 
  tbl(climate.db, "obs_day_climate_data") %>% 
  collect()
```
...
> let's group the days as wet (high rH), average (medium rH), and dry (low rH) days
...
> Groupings created based on rH_max

```{r specify_groups}
clim.groups <-
  tbl(climate.db, "rH_group_climate_data") %>% 
  select(day=date, rH_max=dmax_rH, rH_group) %>% 
  collect()
...
```{r summary_ageclass_v1}
bd.saveplot(
  name = "daily_foraging_pattern_avg",
  width = 25,height = 20,
  
  mod.dat %>% 
  mutate(rH_group=factor(rH_group, levels = clim.group.ids)) %>% 
...
```{r plot_summary_ageclass_v1, echo = FALSE, fig.align='center', fig.cap='', out.width="95%"}
  knitr::include_graphics(paste0(path_to_repo2,
           "daily_foraging_pattern_avg.png"))
```

### Summary Plot 2
> Compare foraging with humidity changes

```{r summary_ageclass_v2}
bd.saveplot(
  name = "daily_foraging_pattern_avg_summary",
  width = 25,height = 20,
  mod.dat %>% 
    mutate(rH_group=factor(rH_group, levels = clim.group.ids)) %>% 
    group_by(age_class, rH_group, t.bin) %>% 
    ...
    ungroup() %>% 
    group_by(age_class, rH_group) %>% 
    mutate(avg_out=(avg_out-median(avg_out))/sd(avg_out),
           avg_rH=(avg_rH-median(avg_rH))/sd(avg_rH)) %>% 
    
    ggplot(aes(x=t.bin, y=avg_out, group=1)) +
          geom_area(aes(x=t.bin, y=avg_rH, group=1),
                    size=3, col=NA, fill="orange", alpha=0.5) +
          geom_line(aes(x=t.bin, y=avg_rH, group=1),
                    size=1, col="black", alpha=0.65) +
          geom_area(position = position_dodge(width = 1),
                    aes(fill=age_class), alpha=0.65) +
          geom_line(size=2, col="black", alpha=0.85) +
          facet_grid(rH_group~age_class) +
    # coord_flip() +
          labs(x="time of day (h)",
               y="",
               title = "orange: humidity\nother colors: foraging") +
          # scale_y_continuous(n.breaks = 4) +
          theme_Publication(base_size = 20) +
          theme(axis.text.x=element_text(angle=90,hjust=1)) +
          scale_y_continuous(breaks = c(-1,0,1)) +
...
```{r plot_summary_ageclass_v2, echo = FALSE, fig.align='center', fig.cap='', out.width="95%"}
  knitr::include_graphics(paste0(path_to_repo2,
           "daily_foraging_pattern_avg_summary.png"))
```

### Summary Plot 3
> Compare foraging with humidity and temperature

```{r summary_ageclass_v3}
bd.saveplot(
  name = "daily_foraging_pattern_avg_summary3",
  width = 25,height = 20,
  mod.dat %>% 
    mutate(rH_group=factor(rH_group, levels = clim.group.ids)) %>% 
    group_by(age_class, rH_group, t.bin) %>% 
    ...
    ungroup() %>% 
    group_by(age_class, rH_group) %>% 
    mutate(avg_out=(avg_out-mean(avg_out))/sd(avg_out),
           avg_rH=(avg_rH-mean(avg_rH))/sd(avg_rH),
    ...
    ggplot(aes(x=t.bin, y=avg_out, group=1)) +
          ...
          geom_line(aes(x=t.bin, y=avg_rH, group=1),
                    size=1, col="black", alpha=0.65) +
          geom_area(aes(x=t.bin, y=avg_temp, group=1),
                    size=3, col=NA, fill="red", alpha=0.5) +
          geom_line(aes(x=t.bin, y=avg_temp, group=1),
                    size=1, col="black", alpha=0.65) +
          geom_area(position = position_dodge(width = 1),
                    # aes(fill=age_class), 
                    fill="grey60",
    ...
    # coord_flip() +
          labs(x="time of day (h)",
               y="",
               title = "orange: humidity, red: temperature\nother colors: foraging") +
          # scale_y_continuous(n.breaks = 4) +
          theme_Publication(base_size = 20) +
          theme(axis.text.x=element_text(angle=90,hjust=1)) +
...
```{r plot_summary_ageclass_v3, echo = FALSE, fig.align='center', fig.cap='', out.width="95%"}
  knitr::include_graphics(paste0(path_to_repo2,
           "daily_foraging_pattern_avg_summary3.png"))
```
...
In this class of analyses, we can replace time of day with humidity, and bin days as low/high/medium humidity days.

> Q1. Do colonies belonging to differet age categories show differences in their foraging?

```{r analyses_v1.1}

mod.full.h <- lme4::lmer(r_out ~ age_class + rH + rH_group + (1|colony_ID) + (1|rH_group:colony_ID),
                 data=mod.dat2, REML=F)
mod.red.h.1 <- lme4::lmer(r_out ~ rH_group + rH + (1|colony_ID) + (1|rH_group:colony_ID),
                 data=mod.dat2, REML=F)
anova(mod.full.h, mod.red.h.1, test="Chisq")
...
> Q2. Does the rate of foraging depend on temporal fluctuations in humidity?

```{r analyses_v2.1}
mod.red.h.2 <- lme4::lmer(r_out ~ age_class + rH_group + (1|colony_ID) + (1|rH_group:colony_ID),
                 data=mod.dat2, REML=F)
anova(mod.full.h, mod.red.h.2, test="Chisq")
```

Ans: No.


> Q3. Does the rate of foraging change between wet/average/dry humidity days?

```{r analyses_v3.1}
mod.red.h.3 <- lme4::lmer(r_out ~ age_class + rH + (1|colony_ID) + (1|rH_group:colony_ID),
                 data=mod.dat2, REML=F)
anova(mod.full.h, mod.red.h.3, test="Chisq")
```
...
```{r analyses_v4.1}
mod.full.h.2 <- lme4::lmer(r_out ~ age_class*rH + rH_group + (1|colony_ID) + (1|rH_group:colony_ID),
                 data=mod.dat2, REML=F)
anova(mod.full.h.2, mod.full.h, test="Chisq")
```
...
rownames(cluster.dat) <- rep("",length(name.rows))
colnames(cluster.dat) <- rep("",length(name.cols))
cluster.dat2 <- matrix(data=0,nrow=length(name.rows), ncol=length(name.cols))
rownames(cluster.dat2) <- rep("",length(name.rows))
colnames(cluster.dat2) <- rep("",length(name.cols))

k <- 1
for(i in 1:length(colonies)) {
  
  foo <- mod.dat %>% filter(colony_ID == colonies[[i]]) %>% select(t.bin, day, r_tot, rH)
  days <- foo %>% pull(day) %>% unique()
