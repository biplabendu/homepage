...
filename <- "arizona_2022_master_list_Aug19_1628h.csv"
...
bead.dat <- 
  read.csv("/Users/biplabendudas/OneDrive - University of Central Florida/UCF de bekker/PostDoc/Labs/Gordon/00_application_package/final_round/data/bead_data_2022/bead_data_18Aug22.csv",
           header = T, stringsAsFactors = F) %>% 
  as_tibble()
bead.colonies <- bead.dat %>% pull(colonyID) %>% unique()
...
```{r for_deborah_4}
colonies.counting.19Aug <- 
  dat %>% 
  filter(date==all.dates[6]) %>%
...
colonies.counting.17Aug <- 
  dat %>% 
  mutate(date = as.POSIXct(date, format="%m/%d/%Y", tz="MST")) %>% 
  filter(date==all.dates[5]) %>%
  select(observer, colony_ID) %>% 
  na.omit() %>%
  pull(colony_ID) %>% 
  unique() %>%
...
# # read list of colonies and indicate which ones are counted
# readxl::read_excel(paste0(path_to_repo, path_to_data,
#                                  "deborah_extra_sheets/which_are_currently_counted.xlsx")) %>% 
#   as_tibble() %>% 
#   select(colony_ID = ID) %>% 
#   mutate(colony_ID = as.factor(colony_ID)) %>% 
#   mutate(counting_17Aug22 = ifelse(colony_ID %in% colonies.counting.17Aug, "yes", "no")) %>% 
#   
#   write.csv(.,
#             file = paste0(path_to_repo,"/04_results/which_are_currently_counted.csv"),
#             row.names = F)


colonies.counting.16Aug <-
  dat %>% 
  mutate(date = as.POSIXct(date, format="%m/%d/%Y", tz="MST")) %>% 
  filter(date==all.dates[4]) %>% 
  select(observer, colony_ID) %>% 
  na.omit() %>% 
  pull(colony_ID) %>% 
  unique() %>%
...
## 19 Aug v. 17 Aug
writeLines("Observed on 19 Aug but NOT on 17 Aug")
dplyr::setdiff(colonies.counting.19Aug, colonies.counting.17Aug)
writeLines("Observed on 17 Aug but NOT on 19 Aug")
dplyr::setdiff(colonies.counting.17Aug, colonies.counting.19Aug)

writeLines("Which of the colonies not observed on 19 Aug are used for the bead experiment?")
dplyr::intersect(dplyr::setdiff(colonies.counting.17Aug, colonies.counting.19Aug), bead.colonies)

# ## 17 Aug v. 16 Aug
# writeLines("Observed on 17 Aug but NOT on 16 Aug")
# dplyr::setdiff(colonies.counting.17Aug, colonies.counting.16Aug)
# writeLines("Observed on 16 Aug but NOT on 17 Aug")
# dplyr::setdiff(colonies.counting.16Aug, colonies.counting.17Aug)
