...
I will use the genderizer.io api for predicting gender from the first names of the participants. The same API is used by BiasWatchNeuro, and is a commonly used tool to estimate gender ratios in a field in the absence of demographic data. The website for the API can be found [here](https://genderize.io/check-gender-csv).
...
Note: Participant names with a count greater than `r count_cutoff` in the database, and a probability of gender given the first name higher than `r prob_cutoff` were used to estimate gender ratio at the two conferences (2014 and 2018). Post-filtering, a total of 461 (out of 576) participants in 2014 and 417 (out of 521) participants in 2018 remained. The estimated number of participants of each gender (male/female) selected for IUSSI 2014 and 2018 are shown below.  
...
DT::datatable(gender.ratio)

```

> Therefore, **the estimated *female:male ratio* **among selected participants at **IUSSI 2014 was `r round(gender.ratio[1,2]/gender.ratio[2,2],2)` and at IUSSI 2018 was `r round(gender.ratio[1,3]/gender.ratio[2,3],2)`.**

***

All code used for this document is available on my [Github](https://github.com/biplabendu/IUSSI_Demography).