...
mod.red.1 <- lme4::lmer(s_cum_prop ~ counted_when + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
anova(mod.full.1, mod.red.1, test="Chisq")


## Let's look at the summary of the model
lmerTest::ranova(mod.full.1)
...
AIC(mod.full.1, mod.full.2)
BIC(mod.full.1, mod.full.2)
...
residuals(mod.full.1) %>% plot(main="Residuals for model mod.full.1")
```
...
```{r mod_linear, echo=F, include=F}
s.tidydat.mod
formula <- s2_cum_prop ~ age_class + counted_when + colonyID
out_lm <- lm(s2_cum_prop ~ age_class + counted_when + colonyID, data=s.tidydat.mod)
anova(out_lm, test="Chisq")
```

### binomial regression

```{r mod_binomial, echo=F, include=F}
# glm(cbind(s.tidydat.mod$s,beads_returned, s.tidydat.mod$beads_collected-s.tidydat.mod$bead_returned) ~ age_class + counted_when + colonyID, 
#     data=s.tidydat.mod %>% mutate(beads_returned = as.integer(beads_collected*cum_prop)),
#                family = binomial(logit))

# Source: https://medium.com/mlearning-ai/time-series-proportion-data-analysis-3ce9071b37ac
