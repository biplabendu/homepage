...
tidydat %>% 
  ...
  geom_smooth(method = "lm", formula = y~log2(x+1), alpha=0.4, size = 2, aes(color=age_class), se=F) +
...
s.tidydat[!is.infinite(rowSums(s.tidydat %>% select(s_cum_prop, s2_cum_prop))),] %>% 
  ...
  # geom_boxplot(alpha=0.3, aes(group=age_class)) +
  # geom_violin(alpha=0.3) +
  # geom_smooth(method = "gam", formula = y~s(x, bs="cs", k=8), aes(group=age_class)) +
  geom_smooth(method = "lm", formula = y~log2(x+1), alpha=0.4, size = 2, aes(color=age_class), se=F) +
  ...
  labs(x = "Day-time of observation",
       y= "cumulative proportion \nof beads returned",
       subtitle = "Line indicates y ~ log2(x+1) + 95% CI",
       caption = "normalized by proportion of maximum # of foragers observed on D1 - V1")
...
#        y= "cumulative proportion \nof beads returned",
#        subtitle = "Line indicates y ~ log2(x+1) + 95% CI",
#        caption = "normalized by proportion of maximum # of foragers observed on D1")
...
s.tidydat[!is.infinite(rowSums(s.tidydat %>% select(s_cum_prop, s2_cum_prop))),] %>% 
  ...
  labs(x = "Day-time of observation",
       y= "cumulative proportion \nof beads returned",
       subtitle = "",
       caption = "normalized by proportion of foragers out on D1 of bead experiments - V1")


```


### Plot normalized bead returns v3
...
s.tidydat[!is.infinite(rowSums(s.tidydat %>% select(s_cum_prop, s2_cum_prop))),] %>% 
  mutate(s3_cum_prop = cum_prop/prop_foraging_30s) %>% 
  mutate(s3_cum_prop = cum_prop/prop_foraging_30s) %>% 
  ...
  ggplot(aes(x=counted_when, y=s3_cum_prop, fill=age_class, color=age_class, group=age_class)) +
  geom_jitter(size=2, alpha=0.4, width = 0.2) +
  # geom_line(aes(group=colonyID), size=0.5, alpha=0.7) +
  # geom_boxplot(alpha=0.3, aes(group=age_class)) +
  # geom_violin(alpha=0.3) +
  ...
  labs(x = "Day-time of observation",
       y= "cumulative proportion \nof beads returned",
       subtitle = "Line indicates y ~ log2(x+1) + 95% CI",
       caption = "normalized by proportion of maximum # of foragers observed on D1 - V2")
...
s.tidydat[!is.infinite(rowSums(s.tidydat %>% select(s_cum_prop, s2_cum_prop))),] %>% 
  ...
  labs(x = "Day-time of observation",
       y= "cumulative proportion \nof beads returned",
       subtitle = "",
       caption = "normalized by proportion of foragers out on D1 of bead experiments - V2")
...
mod.full.1a <- lme4::lmer(s_cum_prop ~ age_class + counted_when + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
mod.full.1b <- lme4::lmer(s_cum_prop ~ age_obs + counted_when + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
mod.full.1c <- lme4::lmer(s_cum_prop ~ age_class*counted_when + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
mod.full.1d <- lme4::lmer(s_cum_prop ~ age_class + poly(as.numeric(counted_when), 2) + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
# Model selection
AIC(mod.full.1a, # best performing model among the three
    mod.full.1b, mod.full.1c, mod.full.1d)

mod.red.1a <- lme4::lmer(s_cum_prop ~ counted_when + (1|colonyID) + (1|bead_color), data=s.tidydat.mod, REML=F)
mod.red.1d <- lme4::lmer(s_cum_prop ~ poly(as.numeric(counted_when), 2) + (1|colonyID) + (1|bead_color),
                        data=s.tidydat.mod, REML=F)
anova(mod.full.1d,  mod.red.1d, test="Chisq")


## Let's look at the summary of the model
lmerTest::ranova(mod.full.1d)
...
afex::mixed(formula(mod.full.1d), data=s.tidydat.mod, check_contrasts = T, test_intercept = F, method="KR")
...
mod.full.2 <- lme4::lmer(s2_cum_prop ~ age_class + poly(as.numeric(counted_when), 2) + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
mod.red.2 <- lme4::lmer(s2_cum_prop ~ poly(as.numeric(counted_when), 2) + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
anova(mod.full.2, mod.red.2, test="Chisq")


## Let's look at the summary of the model
lmerTest::ranova(mod.full.2)

## Calculate the p-values for the fixed effects
## Source: https://mspeekenbrink.github.io/sdam-r-companion/linear-mixed-effects-models.html
afex::mixed(formula(mod.full.2),
                 data=s.tidydat.mod, check_contrasts = T, test_intercept = F, method="KR")


## Checking to see how the model predictions compare against observed values
x <- predict(mod.full.2)
y <- s.tidydat.mod %>% pull(s2_cum_prop)
xy <- data.frame(x,y)
xy %>% 
ggplot(aes(x=x, y=y)) +
  ...
  labs(x="s_cum_prop predicted from mod.full.1",
       y="observed s_cum_prop")

# The slope should be 1
lm(x~y) %>% summary()

# How do the residuals look?
residuals(mod.full.2) %>% plot(main="Residuals for model mod.full.1")
abline(0,0, col="red")
```
...
```{r prelim_analyses_v3}
mod.full.3 <- lme4::lmer(cum_prop ~ age_class + poly(as.numeric(counted_when), 2) + prop_foraging_30s + (1|colonyID) + (1|bead_color) ,
                 data=s.tidydat.mod, REML=F)
...
# How do the residuals look?
residuals(mod.full.3) %>% plot(main="Residuals for model mod.full.3")
abline(0,0, col="red")
```

## Additive models
> using response variable: cum_prop

### model 1: 

cum_prop ~ f(counted_when) + e

```{r gam_v1}
gam.1 <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr"), data = s.tidydat.mod)
## Select model
which.mod <- gam.1
## Summarize model
summary(which.mod)
## Model validation
### using functions from mgcv package
par(mfrow=c(2,2))
gam.check(which.mod)
par(mfrow=c(1,1))
...
cum_prop ~ f(counted_when) + f(age) + e

```{r gam_v2}
gam.2a <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + age_class, data = s.tidydat.mod)
gam.2b <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr", by=age_class) + age_class, data = s.tidydat.mod)
gam.2c <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_obs, k=8, bs="cr"), data = s.tidydat.mod)
## Compare models
AIC(gam.1, gam.2a, gam.2b, gam.2c)
## Select model
which.mod <- gam.2c
## Summarize model
summary(which.mod)
...
par(mfrow=c(1,1))
### manually
mod.validate(which.mod)
```


### model 3: 

cum_prop ~ f(counted_when) + f(age) + f(prop_foraging_30s) + e

```{r gam_v3}
gam.3a <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_obs, k=8, bs="cr") +
                s(prop_foraging_30s, bs="cr"),
                data = s.tidydat.mod)
gam.3b <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_obs, k=8, bs="cr") +
                s(foraging_30s, bs="cr"),
                data = s.tidydat.mod)
gam.3c <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_obs, k=8, bs="cr") +
                s(max_foraging_30s, bs="cr"),
                data = s.tidydat.mod %>% ungroup() %>%  
                  mutate(max_foraging_30s = as.integer(foraging_30s*prop_foraging_30s)))
gam.3bc <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_obs, k=8, bs="cr") +
                s(max_foraging_30s, bs="cr") + s(foraging_30s, bs="cr"),
                data = s.tidydat.mod %>% ungroup() %>%  
                  mutate(max_foraging_30s = as.integer(foraging_30s*prop_foraging_30s)))
## adding interaction term between time and numerical age of observation
gam.3d <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr", by=age_obs) + s(age_obs, k=8, bs="cr") +
                s(max_foraging_30s, bs="cr"),
                data = s.tidydat.mod %>% ungroup() %>%  
                  mutate(max_foraging_30s = as.integer(foraging_30s*prop_foraging_30s)))
## same as gam.3c but using age-class (categorical) instead of age at observation (numerical)
gam.3e <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr") + s(age_class, bs="fs") +
                s(max_foraging_30s, bs="cr"),
                data = s.tidydat.mod %>% ungroup() %>%  
                  mutate(max_foraging_30s = as.integer(foraging_30s*prop_foraging_30s)))
## adding interaction term between time and age-class
gam.3f <- gam(cum_prop ~ s(as.numeric(counted_when), k=8, bs="cr", by=age_class) + s(age_class, bs="fs") +
                s(max_foraging_30s, bs="cr"),
                data = s.tidydat.mod %>% ungroup() %>%  
                  mutate(max_foraging_30s = as.integer(foraging_30s*prop_foraging_30s)))

## Compare models
AIC(gam.1, gam.2c, gam.3a, 
    gam.3b, # seems to be the best performing model so far
    gam.3c, # seems to be the second best performing model so far
    gam.3bc, gam.3d, gam.3e, gam.3f)
## Select model
which.mod <- gam.3bc
## Summarize model
summary(which.mod)
...
# Let's see how this model fits the data
### option 1
vis.gam(which.mod, theta=250, color="heat")
# vis.gam(gam.3e, theta=250, color="heat")
### option 2
s.tidydat.mod %>% 
  mutate(pred = predict(which.mod)) %>% 
  group_by(age_class, counted_when) %>% 
  summarize(mPred = mean(pred)) %>% 
  right_join(s.tidydat.mod) %>% 
  ggplot(aes(x=counted_when)) +
    geom_boxplot(aes(y=cum_prop, fill=age_class), alpha=0.5) +
    # geom_jitter(aes(y=cum_prop, col=age_class), size=2, width=0.3) +
    # geom_line(aes(y=cum_prop, group=interaction(colonyID, bead_color)), col="grey60", alpha=0.5, size=2) +
    geom_line(aes(y=mPred, group=age_class, col=age_class), size=2.5) +
...
There are issues we still need to deal with. (see Zurr 2013, section 2.5)

- Independece issue 1: the residual v. cum_prop graph shows that the residuals are not randomly distributed around the 0,0 line; there is a trend which might be due to temporal correlation present in our data (cum_prop of one time point depends on the cum_prop of previous time point(s)).
- Independece issue 2: the residual v. bead_color graph shows that there is "batch" effects; different replicate (bead_color used) shows slight different distribution of the residuals. We need to add *bead_color* as a random effect (see Zurr 2013 for ideas)
- The model might improve by relaxing the assumption that the residual errors for different age-classes follow the same distribution (here, considered to be normally distributed around 0). As you can see from the histogram of residuals, the distribution is bell-shaped but not centered at 0.(See Zurr, section 2.8 for allowing heterogeniety)
- 
